...
> preserved verbatim. Its still-valid identity/authority semantics (the
> roles/designations/bindings separation, capability floors, the dependency
> matrix and the I-PM invariants) carry into the current revised subset at
> `docs/architecture/participant-model.md`; its L2 "brother authors / worker
> validates-and-publishes" choreography is superseded by ADR-0044
> (single-session unified engineering).
