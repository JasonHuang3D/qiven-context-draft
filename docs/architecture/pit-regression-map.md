...
| P-49 | The draft reimplemented a lower-layer capability: bounds-checked byte IO (`Reader`, `put*` helpers) duplicated `qiven::ByteCursor`/`ByteWriter`/`qiven::endian` with a weaker failure model; the preflight surveyed the draft's own contracts but never Foundation's public surface | foundation/docs/architecture/distillation-2026-09-20-context-draft.md (bucket A, finding A); migration 3790e33 (reads) | PROC | preflight rule: any new capability surveys the lower-layer public surface (Foundation first) before local implementation; duplicated primitives migrate down, never stay local (software-engineering-philosophy 1/7) | preflight rule carried into the Runtime Component ADL Section 90 day-one constraints; result/status vocabulary trigger OBL-20260920T205211Z-D3F7B2 | v4 |
